...
- Use this auxiliary residual scale estimator to compute 
an M-estimator of regression (e.g. using Huber's loss function). 
...
- Compute the L1 regression estimator;
- Compute an M-estimator of the scale of the corresponding residuals;
- Use this auxiliary residual scale estimator and the
L1 regression estimator to start the iterations 
to compute an M-estimator with a re-descending score function. 


#### Random features (explanatory variables)

A solution is to use a bounded loss function, 
...
figures illustrate $$\rho$$ and its derivative (the corresponding
score function):
...
lower the variance), the higher the asymptotic bias). The other 
parameter that can be used to reduce the bias of the estimator
(for a given breakdown point and efficiency), is the family 
of loss functions. The package `RobStatTM` implements the optimal
loss function (`opt`), which can be set using the control 
argument in `lmrobdetMM`. 
Below we revisit the stack loss example, using a 95% efficient
estimator computed with the bias-optimal loss:
